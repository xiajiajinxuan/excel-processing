...
if (Test-Path $venvPython) {
    $python = $venvPython
    Write-Host "[环境] 使用项目虚拟环境: $python" -ForegroundColor Green
}
...
$dependencies = @("PyQt6", "pandas", "openpyxl", "numpy", "PyYAML")

foreach ($dep in $dependencies) {
    $installed = & $python -m pip show $dep 2>&1
    if ($LASTEXITCODE -ne 0) {
        if ($dep -eq "PyQt6") {
            Write-Host "   [错误] PyQt6 未安装，请先执行: $python -m pip install -r requirements.txt" -ForegroundColor Red
            Read-Host "按Enter键退出"
            exit 1
        }
...
$appDir = "dist\Excel数据处理工具"
$exePath = "$appDir\Excel数据处理工具.exe"
if (Test-Path $exePath) {
    $fileInfo = Get-Item $exePath
    $fileSizeMB = [math]::Round($fileInfo.Length / 1MB, 2)
    Write-Host "输出目录: $appDir" -ForegroundColor Green
    Write-Host "可执行文件: $exePath" -ForegroundColor Green
    Write-Host "exe 大小: $fileSizeMB MB ($($fileInfo.Length) 字节)" -ForegroundColor Green
